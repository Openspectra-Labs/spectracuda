...
PHY_KWARGS = dict(
    ...
    # Interleaver -- root-causing the 1024B real-RF `payload_fail` losses
    # (see docs/2026-09-06-rx-packet-loss-and-ism-band-characterization.md
    # and the stream_debug_counts diagnostic below): those failures are
    # RS(rs_m8/fec0)-uncorrectable-codeword ValueErrors (conv_v27/Viterbi
    # itself never raises on a bad channel -- it always produces a best-
    # guess sequence; see spectracuda/fec/viterbi.py), meaning a real
    # over-the-air interference burst is landing as a cluster of Viterbi
    # decode errors concentrated in a narrow byte range that blows RS's
    # per-codeword error budget. `spectracuda/interleaver/block.py`'s own
    # docstring documents this exact scenario and names "block" as "the
    # RECOMMENDED default for this codebase specifically" for it --
    # spreads that narrow burst across many different RS codewords/
    # symbols instead. unit_bits=8 matches rs_m8's byte-symbol size (see
    # interleaver/base.py's docstring -- bit-granularity interleaving of
    # a byte-oriented code was measured to make bursts WORSE, not
    # better). NOT signaled over the air (deliberate design, see
    # framing/packetizer.py's docstring) -- MUST match the TX side's own
    # PHY_KWARGS exactly, same class of requirement as fft_size/n_pilot/
    # n_data/cp_len/sync/cfo above, or every frame fails fec0 decode.
...
# TEMP diagnostic (root-causing 1024B-payload payload_fail losses -- see
# ofdm.py's _decode_payload_from_header() for the full rationale): per-
# OFDM-symbol EVM/pilot-phase/pilot-timing-slope, captured for every
# frame including ones that FAIL payload decode. Meant to distinguish
# external interference (a sudden jump late/mid-frame) from residual
# CFO/timing drift or the stale-channel-estimate gap (a gradual trend
# across the frame) -- see the summary block this script prints at the end.
ofdm_rx.debug_payload_symbols = True
...
n_crc_valid = 0
n_consumed = 0
completed_symbol_diagnostics = []  # TEMP diagnostic -- one entry per COMPLETED frame, for contrast against stream_failed_symbol_diagnostics
...
while time.perf_counter() < t_end:
    ...
    if result is not None:
        ...
        if result.get("symbol_diagnostics") is not None:
            completed_symbol_diagnostics.append(result["symbol_diagnostics"])
...
# remove this block once that investigation is closed out): breaks down
# every candidate frame rx_streaming() gave up on, by WHERE it gave up.
dbg = ofdm_rx.stream_debug_counts
print(f"[stream_debug] sync_triggers (SEEKING->WAITING_HEADER): {dbg['sync_triggers']}  "
      f"header_fail: {dbg['header_fail']}  payload_fail: {dbg['payload_fail']}  "
      f"completed: {n_decoded}  "
      f"(sync_triggers should = header_fail + payload_fail + completed)")

# TEMP diagnostic (root-causing WHICH mechanism -- external interference vs
# residual CFO/timing drift vs the stale-channel-estimate gap -- explains
# payload_fail; see ofdm.py's _decode_payload_from_header() for the full
# rationale). Per-frame summary of the per-OFDM-symbol readouts:
#   argmax(evm) near the END of the frame, consistently across failed
#   frames -> points at gradual drift (CFO/timing/stale channel estimate).
#   argmax(evm) scattered at random positions frame to frame -> points at
#   a discrete external event (interference) hitting an arbitrary point,
#   not a trend internal to the frame itself.
def _summarize_symbol_diagnostics(label, diag_list):
    if not diag_list:
        print(f"  [symbol_diag] {label}: none captured")
        return
    argmax_fracs = []
    for i, d in enumerate(diag_list):
        if d is None:
            continue
        evm_s = np.asarray(d["evm_per_symbol"])
        cpe_s = np.unwrap(np.asarray(d["pilot_cpe_per_symbol"]))
        slope_s = np.asarray(d["pilot_timing_slope_per_symbol"])
        n = len(evm_s)
        if n < 2:
            continue
        argmax_idx = int(np.argmax(evm_s))
        argmax_frac = argmax_idx / (n - 1)
        argmax_fracs.append(argmax_frac)
        cpe_drift = float(cpe_s[-1] - cpe_s[0])  # total accumulated phase (rad) across the frame -- CFO-drift signature
        if i < 10:  # full detail for the first 10 only -- rest folded into the aggregate below
            print(f"  [symbol_diag] {label} #{i}: n_symbols={n} evm(min/med/max)="
                  f"{evm_s.min():.4f}/{np.median(evm_s):.4f}/{evm_s.max():.4f} "
                  f"argmax_at={argmax_frac:.2f}(frac of frame) cpe_drift={cpe_drift:+.3f}rad "
                  f"timing_slope(min/max)={slope_s.min():+.2e}/{slope_s.max():+.2e}")
    if argmax_fracs:
        arr = np.asarray(argmax_fracs)
        print(f"  [symbol_diag] {label} AGGREGATE over {len(arr)} frames: "
              f"argmax(evm) position mean={arr.mean():.2f} std={arr.std():.2f} "
              f"(near 1.0+low std -> gradual drift signature; scattered/high std -> discrete-event signature)")

_summarize_symbol_diagnostics("FAILED", ofdm_rx.stream_failed_symbol_diagnostics)
_summarize_symbol_diagnostics("completed", completed_symbol_diagnostics)
